...
func (r *TodoListPostgres) Delete(userId, listId int) error {
	query := fmt.Sprintf("DELETE FROM %s USING %s ul WHERE tl.id = ul.list_id "+
		"WHERE ul.user_id = $1 AND ul.list_id = $2",
		todoListsTable, usersListsTable)
...
func (r *TodoListPostgres) Update(userId, listId int, input entities.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id = ul.list_id "+
		"WHERE ul.user_id = $%d AND ul.list_id = $%d", todoListsTable, setQuery, usersListsTable, argId, argId+1)
